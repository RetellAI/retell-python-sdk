...
class MessageMessageBaseMultimedia(BaseModel):
    url: str
...
class MessageMessageBase(BaseModel):
    ...
    multimedia: Optional[List[MessageMessageBaseMultimedia]] = None
...
class PlaygroundCompletionResponse(BaseModel):
    ...
    Same shape as chat completion response messages. Does not include the input
    messages.
    """

    call_ended: Optional[bool] = None
    ...
    knowledge_base_retrieved_contents: Optional[List[str]] = None
